...
export const syncAccounts = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        for (const zAccount of zernioAccounts) {
            ...
            const normalizedPlatform = supportedPlatforms.find(p => rawPlatform.includes(p.toLowerCase()))||null;

            if(!normalizedPlatform){
                console.log(`skipping unsupported platform ${normalizedPlatform} account`)
                continue;
            }
